...
- GitHub-hosted Actions execution recovered after the repository became public; this child branch exists only to obtain a clean mergeable PR trigger for the unchanged E27 workflow.
...
- **E27 prior execution status**: earlier runs failed before any workflow step while the repository was private. PR #30 is currently non-mergeable against its historical base, so its `pull_request` workflow is not a reliable trigger even after public runner recovery.
- **E27 public child run**: run the unchanged E27 experiment from a clean child PR based on the E27 branch. No modeling or experiment parameter changes are introduced.
...
## Next
1. Inspect the public E27 artifact for all 7/8/10-stage × 256/512/1024 cases.
