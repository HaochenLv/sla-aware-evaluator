...
- **E27 execution status**: original Actions run `32701638835` failed before any workflow step. A fresh rerun of run `32701691518` also failed before any step; latest job `97357373392` again has `steps=null` and no log URL. Therefore E27 still has no HELIX runtime evidence. Do not count per-stage scaling as validated.
...
## Next
Keep E27 pending and rerun the nine direct HELIX cases when Actions runners recover. Until then, the strongest completed evidence is E26's in-configuration cross-experiment transfer; do not extrapolate it to stage counts 7/10. E28 separately audits exposure timing using archived successful E10 data, so progress can continue without treating the runner outage as model evidence.
