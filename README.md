...
- 有限 workload 的时间压缩/扩张和 capacity 二分搜索；
...
固定 120 秒窗口生成 133 个有限请求，当前结果为：

| Placement | Safe intensity 下界 | Safe arrival rate 下界 | 首个瓶颈 |
|---|---:|---:|---|
| 2.5 Gbps 慢链路顺序 | 0.4453 | 0.512 req/s | Network |
| 10 Gbps 快链路顺序 | 1.2734 | 1.463 req/s | Network |

两个候选使用相同的 8 个 A100、相同的 80 层切分和相同 workload，只改变 Stage 到物理节点的排列及由此经过的链路。快链路 placement 的保守 capacity 下界约为慢链路 placement 的 2.86 倍。
