...
def overhead_interval_s_per_token(
    ...
    # Unsafe lower bandwidth gives TTFT > SLA, so the unknown overhead must be
    # larger than the budget left after compute/network/fixed terms. Safe upper
    # bandwidth gives TTFT <= SLA, so it gives the corresponding upper bound.
    lower = (
        TTFT_S
        - FIXED_OVERHEAD_S
        - prefill_compute_s
