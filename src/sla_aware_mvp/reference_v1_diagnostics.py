...
from .reference_v1 import evaluate_reference_v1


def diagnose_reference_v1_tpot(
    *,
    pipeline: Pipeline,
    ...
    """Exact first-failing-token attribution for Reference v1.

    This mirrors the v0 diagnostic and changes only the evaluator being traced.
    Queue/service accounting therefore remains directly comparable across v0/v1.
    """
    ...
    original = evaluate_reference_v1(
        pipeline=pipeline,
        workload=scaled,
        sla=sla,
        profiler=tracing,
    ...
    if request_id is not None and observed_tpot_s is not None and violation is not None:
        ...
        if output_token_index is None:
            raise RuntimeError("could not identify failing Decode token from Reference v1 trace")
        ...
        profile_batch_sizes = tuple(record.decode_batch_size for record in token_records)
    ...
    counterfactual = evaluate_reference_v1(
    ...
    if (
        ...
        if counterfactual_token_index is None:
            raise RuntimeError(
                "could not identify counterfactual failing Decode token from Reference v1 trace"
            )
    ...
    if attribution is None:
        ...
        accounted_s = attribution.gpu_service_elapsed_s + attribution.link_service_elapsed_s + overhead_s
    ...
    if profile_batch_sizes and trace_batch_sizes != profile_batch_sizes:
        raise RuntimeError("profile and Reference v1 trace Decode batch sizes disagree")
