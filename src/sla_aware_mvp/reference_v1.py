...
def evaluate_reference_v1(
    *,
    ...
    """Explicit-service Reference v1 with deterministic Decode cohorts.

    Relative to Reference v0, only Decode batching is changed. A Decode token
    becomes ready at the end of Prefill or the previous token. When stage 0 can
    next serve Decode work, all currently eligible ready tokens ahead of the next
    queued compute operation form one cohort. That cohort keeps the same members
    through every pipeline stage; a downstream stage starts only after all cohort
    members have reached it. New requests that become ready after cohort launch
    wait for a later cohort.

    Prefill remains FIFO, physical links remain per-request FCFS D/B servers,
    HELIX/other profiler values are unchanged, and TTFT/TPOT are still observed
    from the explicit execution trajectory. This is a deterministic reference
    policy, not a claim to reproduce a production runtime exactly.
    """
    ...
    def emit_token(
        *, event: str, request_id: str, token_index: int, operation_seq: int | None
    ) -> None:
        if trace_sink is None:
            return
        trace_sink.record(
        ...
        *, request_id: str, token_index: int, batch_id: int, stage_index: int
        ...
        if all(member_id in arrived for member_id in expected):
            ...
            node_servers[stage.node_id].queue.append(
                _DecodeBatch(batch_id=batch_id, stage_index=stage_index, members=members)
            )
    ...
    def start_node_item(node_id: str, item: _ComputeOp | _DecodeBatch) -> None:
    ...
    def start_idle_servers() -> None:
        ...
        for node_id in sorted(node_servers):
            ...
            if node_id == stage0_node_id and ready_decode:
                ...
                if eligible:
                    ...
                    batch_expected[batch_id] = tuple(op.request_id for op in members)
        ...
        for link_id in sorted(link_servers):
            ...
            server.busy = True
            state = states[op.request_id]
            transfer_tokens = state.spec.input_tokens if op.phase == Phase.PREFILL else 1
    ...
    while arrival_index < len(requests) or states or completion_heap:
        ...
        if next_time == float("inf"):
            if states:
                raise RuntimeError("reference v1 deadlocked with active requests")
            break
        ...
        for _, _, event_kind, resource_kind, resource_id, payload in due:
            ...
            if resource_kind == "node":
                ...
                if op.route_position + 1 < len(op.route_link_ids):
                    ...
                    if op.batch_id is None or op.token_index is None:
                        raise RuntimeError("decode link operation missing batch/token identity")
                    arrive_decode_stage(
                        request_id=op.request_id,
                        token_index=op.token_index,
                        batch_id=op.batch_id,
...
def find_reference_capacity_v1(
    *,
    pipeline: Pipeline,
    workload: Sequence[RequestSpec],
    sla: SLA,
    ...
    """Capacity search for Reference v1 using the same sampled-frontier policy."""
